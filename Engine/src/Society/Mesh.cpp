...
namespace sc {
    ...
    void Mesh::Draw(const sc::Shader &shader) const {
        ...
        glDrawElements(
            GL_TRIANGLES,
            static_cast<GLint>(indices.size()),
            GL_UNSIGNED_INT,
            nullptr
            );
        glBindVertexArray(0);
    ...
    void Mesh::setupMesh() {
        ...
        glBufferData(
            GL_ARRAY_BUFFER,
            static_cast<GLint>(vertices.size() * sizeof(Vertex)),
            &vertices[0],
            GL_STATIC_DRAW
            );
        ...
        glBufferData(
            ...
            static_cast<GLint>(indices.size() * sizeof(unsigned int)),
            &indices[0],
            GL_STATIC_DRAW
            );
